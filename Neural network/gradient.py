...
def gradient(inputVec: list, functionNumber: int) -> str:
    ...
    while True:
        ...
        if functionNumber == 1:  # obliczanie wartosci jesli badamy funkcje 1
            ...
            newVector[0] = inputVec[0] - c_const * (
                lambda vector: 2 * pow(vector[0], 3) - pow(vector[0], 2) - vector[0]
            )(inputVec)
            # stała c przemnożona przez pochodna dla x2 funkcji drugiej
            newVector[1] = inputVec[1] - c_const * (lambda vector: 2 * vector[1] - 2)(
                inputVec
            )
        ...
        # w zależności która funkcje sprawdzalismy
        if maxVart < eps_const:
            if functionNumber == 1:
                return (
                    f"Dla parametrów: x1 = {newVector[0]}, x2 = {newVector[1]},"
                    + f"funkcja F1 ma wartość: {round(2*pow(newVector[0],2) + pow(newVector[1],2) - 2*newVector[0]*newVector[1] - 2*newVector[1] + 1,4)}"
                )
            elif functionNumber == 2:
                return (
                    f"Dla parametrów: x1 = {newVector[0]}, x2 = {newVector[1]},"
                    + f"funkcja F2 ma wartość: {round(pow(newVector[0],4)/2 - pow(newVector[0],3)/3 - pow(newVector[0],2)/2 + pow(newVector[1],2) - 2*newVector[1] + 1, 4)}"
                )
